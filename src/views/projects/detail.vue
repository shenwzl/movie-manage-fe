...
<template>
  <div class="dashboard-container">
    ...
    <div v-if="step === '2'" class="shooting-info">
      <h3>拍摄费用</h3>
      <el-switch
        v-model="isHide"
    ...
    <div v-if="step === '3'" class="last-info">
      <h3>后期费用</h3>
      <el-switch
        v-model="isHide"
        style="margin-bottom: 15px;"
...
export default {
    ...
    this.step === '1' &&
      this.getBaseInfo(this.pId).then(res => {
        ...
        const baseInfo = [
          ...
          {
            label: '客户所属公司一级',
            value: this.getCompanyName(data.companyId, this.allCompanys)
          },
          {
            label: '客户所属公司二级',
            value:
              data.childCompanyId && this.getCompanyName(data.childCompanyId, this.allCompanys)
          },
  ...
  methods: {
    ...
    onHideShootChange(val) {
      if (val) {
        this.shootingInfo = this.responseFees.filter(
          fee => fee.budgetAmount !== 0 || fee.realAmount !== 0
        )
      } else {
    ...
    onHideLastChange(val) {
      if (val) {
        ...
        this.lastStateInfo = this.responseFees.filter(
          fee => fee.budgetAmount !== 0 || fee.realAmount !== 0
        )
      } else {
